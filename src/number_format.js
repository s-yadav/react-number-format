...
class NumberFormat extends React.Component {
  ...
  getNumberRegex(g: boolean, ignoreDecimalSeparator?: boolean) {
    const { format, decimalScale, convertPersianDigits } = this.props;
    const { decimalSeparator } = this.getSeparators();
    return new RegExp(
      '[0-9' +
  ...
  formatNumString(numStr: string = '') {
    const { format, allowEmptyFormatting, convertPersianDigits } = this.props;
    let formattedValue = convertPersianDigits ? convertPersianDigitsToEnglish(numStr) : numStr;
