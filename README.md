...
| type | One of ['text', 'tel', 'password'] | text | Input type attribute. Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
| format | String : Hash based ex (#### #### #### ####) <br/> Or Function| none | If format given as hash string allow number input inplace of hash. If format given as function, component calls the function with unformatted number and expects formatted number. Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
| removeFormatting | (formattedValue) => numericString | none | If you are providing custom format method and it add numbers as format you will need to add custom removeFormatting logic. Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
| mask | String (ex : _) | `' '` | If mask defined, component will show non entered placed with masked value. Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
| customInput | Component Reference | input | This allow supporting custom inputs with number format. Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
| onValueChange | (values) => {} | none | onValueChange handler accepts [values object](#values-object). Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
| isAllowed | ([values](#values-object)) => true or false | none | A checker function to check if input value is valid or not. If this function returns false, the onChange method will not get triggered. Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
| renderText | (formattedValue, customProps) => React Element | null | A renderText method useful if you want to render formattedValue in different element other than span. It also returns the custom props that are added to the component which can allow passing down props to the rendered element. Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
| getInputRef | (elm) => void | null | Method to get reference of input, span (based on displayType prop) or the customInput's reference. See [Getting reference](#getting-reference). Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format)
| allowedDecimalSeparators | array of char | none | Characters which when pressed result in a decimal separator. When missing, decimal separator and '.' are used. Here is a [demo](http://localhost:8888/23/decimalSeparator?source=react-number-format) |
