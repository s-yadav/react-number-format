...
function roundToPrecision(numStr, precision) {
  ...
  console.log(hasDecimalSeperator);
  const currentDecimalPart = numberParts[1] || '';
  const roundedDecimalParts = currentDecimalPart.length > precision
  ...
  _createClass(NumberFormat, [{
    ...
    value: function formatInput(val) {
      ...
      if (format) {
        ...
        var hasDecimalSeparator = formattedValue.indexOf(decimalSeparator) !== -1;
